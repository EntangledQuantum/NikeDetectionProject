...
from digital_air_cv.config.loader import DEFAULT_CONFIG_PATH, load_config
from digital_air_cv.config.schema import PATTERN_VALUES, SENSITIVITY_LEVELS, RunSettings
from digital_air_cv.io.extract import extract_legacy, extract_new_pattern
from digital_air_cv.pipeline.registry import ALL_DETECTOR_KEYS
from digital_air_cv.pipeline.runner import collect_image_files, run_paths
from digital_air_cv.pipeline.types import ImageType, classify_image
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m digital_air_cv -i Cyan_full.tiff --pattern new --regions-only
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if os.path.isdir(input_path):
        ...
        if kind == ImageType.FULL and not args.extract:
            try:
                from digital_air_cv.pipeline.runner import manual_regions
