...
class DebrisIslandDetector(BaseDetector):
    ...
    def scan_from_left(self, binary_image):
        ...
        for y_key in sorted(lines_by_y.keys()):
            ...
            if len(line_group) >= self.min_detection_count:
                # Use the first detection (leftmost) as the line position
                candidate_line = line_group[0]
                
                # Check minimum distance from previous line
                if last_y is None or abs(candidate_line['y'] - last_y) >= self.min_distance:
                    filtered_lines.append(candidate_line)
                    last_y = candidate_line['y']
                    if self.debug:
                        print(f"Left line at Y≈{y_key}: {len(line_group)} detections, using first at x={candidate_line['x']}, y={candidate_line['y']}")
                elif self.debug:
                    print(f"Left line at Y≈{y_key}: Skipped - too close to previous line (distance={abs(candidate_line['y'] - last_y)} < {self.min_distance})")
    ...
    def scan_from_right(self, binary_image):
        ...
        for y_key in sorted(lines_by_y.keys()):
            line_group = lines_by_y[y_key]
            if len(line_group) >= self.min_detection_count:
                # Use the first detection (rightmost) as the line position
                candidate_line = line_group[0]
                ...
                if last_y is None or abs(candidate_line['y'] - last_y) >= self.min_distance:
                    ...
                    last_y = candidate_line['y']
                    if self.debug:
                        print(f"Right line at Y≈{y_key}: {len(line_group)} detections, using first at x={candidate_line['x']}, y={candidate_line['y']}")
                elif self.debug:
                    print(f"Right line at Y≈{y_key}: Skipped - too close to previous line (distance={abs(candidate_line['y'] - last_y)} < {self.min_distance})")
    ...
    def draw_lines(self, image, matched_lines):
        """Draw only the detected lines (main visualization)"""
        if len(image.shape) == 2:
        ...
        for match in matched_lines:
            ...
            # Draw the line from left detection to right detection
            cv2.line(vis, (left_pt['x'], left_pt['y']), 
                    (right_pt['x'], right_pt['y']), 
                    (0, 255, 0), self.line_thickness)
            
            # Draw dots at detection points
            cv2.circle(vis, (left_pt['x'], left_pt['y']), 5, (255, 0, 0), -1)  # Blue dot on left
            cv2.circle(vis, (right_pt['x'], right_pt['y']), 5, (0, 0, 255), -1)  # Red dot on right
    ...
    def detect(self, image):
        ...
        # Create main visualization (just lines)
        visualization = self.draw_lines(image, matched_lines)
        ...
        if matched_lines:
            ...
            cv2.putText(visualization, "No matching lines detected", 
    ...
    def save_debug_images(self, output_dir, base_name):
        ...
        if self.debug and hasattr(self, '_debug_kernel_image') and self._debug_kernel_image is not None:
